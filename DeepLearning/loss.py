# Video 8 Implementing the loss calculation:
import numpy as np
...
class LossCategoricalCrossentropy(Loss):
    def forward(self, y_pred, y_true):
        ...
        y_pred_clipped = np.clip(y_pred, 1e-7, 1 - 1e-7)  # To prevent the data from being divided by 0

        if len(y_true.shape) == 1:  # Video 8: 11:00 - 14:00
            correct_confidences = y_pred_clipped[range(samples), y_true]

        elif len(y_true.shape) == 2:
            correct_confidences = np.sum(y_pred_clipped * y_true, axis=1)

        loss = -np.log(correct_confidences)  # Calculating the overall loss
        return loss

    def backward(self, dvalues, y_true):  # S.213 - 215
